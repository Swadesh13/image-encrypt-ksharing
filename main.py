...
def main():
    ...
    group.add_argument("--key", type=str, help="Key for encrypting image (min size = 2*n + 1 digits). Can be alphanumeric.")
    ...
    parser.add_argument("-k", type=int, default=3, help="Value of k in (n, k) sharing method.")
    ...
    if not key.isalnum():
        parser.error("Key must be alphanumeric.")
    elif not key.isnumeric():
        key = str(int(key, 36))
    if len(key) < 2 * args.n + 1:
        parser.error("Int key length should be at least equal to 2n. Given length:", len(key))
